...
AMD and frozen as `v1.0.0`. The 2026-09-04 result is `READY WITH FOLLOW-UPS`: the boot, storage, security, network, power, desktop, backup, and recovery baseline is ready for daily use. Chapter 16 adds an optional reviewed AUR workflow and awaits hardware validation; Qt theming, automatic idle suspend, Plymouth, TPM2-bound unlock, advanced desktop polish, and per-host output overrides remain deliberate later work.
